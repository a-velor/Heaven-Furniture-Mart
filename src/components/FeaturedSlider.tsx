...
interface FeaturedSliderProps {
  onOpenQuote: (itemTitle?: string) => void;
}

export const FeaturedSlider: React.FC<FeaturedSliderProps> = ({ onOpenQuote }) => {
  ...
  // Take the 5 flagship pieces for the featured slider
  const featuredItems: FurnitureItem[] = COLLECTIONS.slice(0, 5);
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 items-stretch min-h-[460px]">
              ...
              <div className="relative aspect-[16/10] sm:aspect-[16/11] lg:h-full w-full overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  ...
                  {currentItem.highlightTag && (
                    <span className="px-3 py-1 bg-[#C5A880] text-[#132629] text-xs uppercase font-bold tracking-wider">
                      {currentItem.highlightTag}
                    </span>
                  )}
                </div>
            ...
            <div className="lg:col-span-5 p-6 sm:p-8 lg:p-10 flex flex-col justify-between bg-white dark:bg-[#122022] transition-colors">
              ...
              {/* Action Buttons */}
              <div className="pt-4 border-t border-stone-200 dark:border-[#C5A880]/20 flex flex-col sm:flex-row gap-3">
                <button
                  id={`slider-inquire-${currentItem.id}`}
                  onClick={() => onOpenQuote(currentItem.name)}
                  className="flex-1 py-3.5 px-6 bg-[#132629] dark:bg-[#C5A880] hover:bg-[#1B3236] dark:hover:bg-[#D4B78F] text-[#FAF8F5] dark:text-[#132629] text-xs font-semibold uppercase tracking-wider flex items-center justify-center gap-2 btn-luxury shadow-md"
                >
                  <span>Inquire for Custom Blueprint</span>
                  <ArrowRight className="w-4 h-4 text-[#C5A880] dark:text-[#132629]" />
                </button>

                <a
                  href={`https://wa.me/8801960481983?text=${encodeURIComponent(`Hello Heaven Furniture Mart, I am inquiring about the ${currentItem.name} featured in your master commissions gallery.`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="py-3.5 px-5 bg-[#FAF8F5] dark:bg-[#162528] hover:bg-stone-100 dark:hover:bg-[#1e3236] text-[#132629] dark:text-[#FAF8F5] border border-stone-300 dark:border-[#C5A880]/30 text-xs font-semibold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors"
                >
            ...
            <div className="flex items-center gap-3 overflow-x-auto w-full sm:w-auto pb-1 sm:pb-0">
              {featuredItems.map((item, idx) => (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 text-xs font-medium transition-all text-left whitespace-nowrap border ${
                    currentIndex === idx
                      ? 'bg-white dark:bg-[#1B3236] border-[#132629] dark:border-[#C5A880] text-[#132629] dark:text-[#FAF8F5] shadow-sm'
