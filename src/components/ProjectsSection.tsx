...
import { Sparkles, MapPin, Calendar, ArrowRight, Eye, Check, Layers, Sliders, ExternalLink } from 'lucide-react';
...
export const ProjectsSection: React.FC<ProjectsSectionProps> = ({ onOpenQuote }) => {
  ...
  const [modalProject, setModalProject] = useState<CompletedProject | null>(null);
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-4 mb-8 scrollbar-none border-b border-stone-200 dark:border-stone-800">
          {COMPLETED_PROJECTS.map((project, idx) => (
            <button
              ...
              className={`px-4 py-2.5 text-xs font-semibold uppercase tracking-wider whitespace-nowrap transition-all rounded-sm flex items-center gap-2 ${
                activeProjectIndex === idx
                  ? 'bg-[#132629] dark:bg-[#C5A880] text-[#FAF8F5] dark:text-[#132629] shadow-md'
                  : 'bg-white dark:bg-[#122224] text-stone-600 dark:text-stone-400 hover:text-[#132629] dark:hover:text-[#FAF8F5] border border-stone-200 dark:border-stone-800'
              }`}
            >
              <span>{project.category}</span>
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                activeProjectIndex === idx
                  ? 'bg-[#C5A880]/30 dark:bg-[#132629]/20 text-[#FAF8F5] dark:text-[#132629]'
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {COMPLETED_PROJECTS.map((project, idx) => (
              <div
                ...
                className={`group cursor-pointer p-3 bg-white dark:bg-[#122224] border rounded-sm transition-all hover:shadow-md ${
                  activeProjectIndex === idx
