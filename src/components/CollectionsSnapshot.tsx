...
import { Sparkles, ArrowRight, Eye, Check, X, ShieldCheck, Share2 } from 'lucide-react';
...
export const CollectionsSnapshot: React.FC<CollectionsSnapshotProps> = ({ onOpenQuote }) => {
  const [activeCategory, setActiveCategory] = useState<string>('all');
  const [selectedItem, setSelectedItem] = useState<FurnitureItem | null>(null);
  const [itemToShare, setItemToShare] = useState<FurnitureItem | null>(null);
  ...
  return (
    <section id="collections" className="py-20 lg:py-28 bg-[#FAF8F5] dark:bg-[#0B1617] text-[#2C221E] dark:text-[#F2EFE9] relative border-t border-stone-200/60 dark:border-stone-800 transition-colors duration-300">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-4 mb-10 no-scrollbar border-b border-stone-200 dark:border-stone-800">
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 text-xs uppercase tracking-wider font-semibold whitespace-nowrap transition-all border-b-2 -mb-[1px] ${
                activeCategory === cat.id
                  ? 'border-[#132629] dark:border-[#C5A880] text-[#132629] dark:text-[#FAF8F5]'
          ...
          {filteredItems.map((item) => (
            ...
            >
              <div>
                ...
                <div className="relative aspect-[4/3] overflow-hidden bg-stone-100 dark:bg-stone-900">
                  ...
                  <div className="absolute top-3 left-3 flex gap-2">
                    ...
                    {item.highlightTag && (
                      <span className="px-2 py-1 bg-[#C5A880] text-[#132629] text-[10px] uppercase font-bold tracking-wider">
                        {item.highlightTag}
                      </span>
                    )}
                  </div>
              ...
              <div className="px-6 pb-6 pt-2 border-t border-stone-100 dark:border-stone-800 flex items-center justify-between">
                <button
                  onClick={() => setSelectedItem(item)}
                  className="text-xs font-semibold text-stone-600 dark:text-stone-400 hover:text-[#132629] dark:hover:text-[#C5A880] transition-colors"
                >
                  View Details
                </button>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 text-xs uppercase tracking-wider font-semibold text-[#8C6239] dark:text-[#C5A880] hover:text-[#132629] dark:hover:text-[#FAF8F5] transition-colors group/btn"
                >
