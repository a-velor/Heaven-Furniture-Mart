...
import { X, Copy, Check, MessageCircle, Facebook, Twitter, Linkedin, Sparkles, Image as ImageIcon, Eye, EyeOff, Download } from 'lucide-react';
...
export const SocialShareModal: React.FC<SocialShareModalProps> = ({
  ...
  const [dynamicOgUrl, setDynamicOgUrl] = useState<string>('');
  const [isGenerating, setIsGenerating] = useState(true);
  const [isCardPreviewVisible, setIsCardPreviewVisible] = useState(true);

  // Generate dynamic canvas-based OG Card preview with safe fallback
  useEffect(() => {
    if (!isOpen) return;
    ...
    generateDynamicBespokeOgImage({
      ...
      .then((dataUrl) => {
        if (isMounted) {
          ...
          // Update live document head metadata with the dynamic bespoke card
          applySeoMetadata({
            title,
            description,
            ogImage: dataUrl,
          ...
          <div className="border border-[#C5A880]/40 bg-[#0E1A1C] p-3 shadow-inner rounded-sm">
            <div className="flex items-center justify-between text-[11px] text-[#C5A880] font-semibold uppercase tracking-wider mb-2 px-1">
              ...
              <div className="flex items-center gap-2">
                {isGenerating ? (
                  <span className="text-stone-400 animate-pulse text-[10px]">Rendering Preview...</span>
                ) : (
            ...
            {isCardPreviewVisible ? (
              ...
              <div className="py-2.5 px-3 bg-stone-900/80 border border-stone-800/80 flex items-center justify-between text-xs text-stone-400">
