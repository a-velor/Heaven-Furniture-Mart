...
// High-fidelity luxury placeholder fallback during chunk loading
const SectionFallback: React.FC = () => (
  <div className="py-24 bg-[#FAF8F5] dark:bg-[#0B1617] flex items-center justify-center">
...
export default function App() {
  ...
  const [selectedRoomForQuote, setSelectedRoomForQuote] = useState<string>('Living Room Suite');

  const handleOpenQuote = (roomOrPiece?: string) => {
    if (roomOrPiece) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#FAF8F5] dark:bg-[#0B1617] text-[#2C221E] dark:text-[#F2EFE9] antialiased selection:bg-[#C5A880]/30 selection:text-[#132629] dark:selection:text-[#FAF8F5] transition-colors duration-300">
      ...
      <main className="flex-1">
        {/* 1. Hero Section (Eagerly loaded with Parallax effect & High-contrast typography) */}
        <Hero onOpenQuote={() => handleOpenQuote()} />

        {/* Below-the-fold components split into dynamic bundles for Lighthouse 90+ */}
        <Suspense fallback={<SectionFallback />}>
          {/* 2. Interactive Featured Suites JavaScript Slider */}
          ...
          {/* 5. Curated Collections Snapshot Gallery */}
          ...
          {/* 7. Social Proof, Founder's MD Statement & Milestones */}
          <StoryAndProof />

          {/* 8. Agrabad Flagship Showroom & Unified Call-to-Action */}
          <ShowroomLocation onOpenQuote={() => handleOpenQuote()} />
        </Suspense>
      </main>

      {/* 9. Footer with full contact, navigation & socials */}
